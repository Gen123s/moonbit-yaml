...
- [x] **注释保留的解析与发射**（`parse_commented` / `CNode`）：修改值后写回，
  所有注释与空行原样保留
...
常规的 解析 → 修改 → 序列化 会丢掉全部注释。`parse_commented` 把注释挂在
语法树上，修改配置后写回时每条注释、空行、锚点和合并键都原样还原：

```moonbit
let doc = @yaml.parse_commented(source)
doc.root.set_path("services.web.image", Str("caddy:2"))
print(doc.to_yaml_string()) // 注释完好，只有值变了
```
